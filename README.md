...
AI triage (Google Gemini), email notifications, rate limiting, file logging, and metrics.
...
| Var | Purpose | Default |
|-----|---------|---------|
| `GEMINI_API_KEY` | Enables AI triage; omit to use rule-based fallback. Free key: https://aistudio.google.com/apikey | (none) |
| `AI_MODEL` | Gemini model id | `gemini-2.5-flash` |
| `AI_TIMEOUT_SECONDS` | Max seconds to wait for the Gemini response | `12.0` |
...
- **AI:** Google Gemini via the `google-genai` SDK — structured JSON output via `response_schema` (`gemini-2.5-flash`, free tier)
...
One Gemini API call per contact submission returns a structured JSON object guaranteed by `response_schema` (the SDK constrains the model to a Pydantic schema, so the reply is always valid JSON):
...
- Gemini returns an error or times out (12 s default)
...
> **Build-time vs runtime:** the project was *built* with Claude (Anthropic) as the development tool, while the *runtime* AI feature (contact triage) calls **Google Gemini**. Two different uses of AI — one to write the code, one inside the product.
...
- **Manual fixes:** Integration issues that required cross-task context: logging lifecycle (logger initialized before middleware fires), middleware ordering (CORS before RequestLogging), dependency injection wiring (`app/dependencies.py`), and swapping the runtime AI provider to Gemini (`response_schema` structured output, no-default schema model per googleapis/python-genai#699).
...
4. Set the secret env vars (`GEMINI_API_KEY`, `SMTP_USER`, `SMTP_PASSWORD`, `OWNER_EMAIL`) in the Render dashboard — they are marked `sync: false` and must be entered manually.
